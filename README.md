...
Because the agent directly appends user-provided audit notes to the system prompt without sanitization, it is susceptible to prompt injection. An attacker could potentially override the "Audit Agent" instructions to extract internal system logic. Additionally, the rapid "vibe coding" phase makes it incredibly easy to hardcode API keys directly into the source.
